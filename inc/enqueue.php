<?php
    ...
    // Load CSS
    function ignitepreservation_load_styles() {
        // Register Styles
        wp_register_style('font-awesome_style', '//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css');
        wp_register_style('ignitepreservation_main_style', get_template_directory_uri() . '/assets/styles/main.css');

        // Enqueue Styles
        wp_enqueue_style( 'wp-theme-boilerplate-style', get_stylesheet_uri() );
        wp_enqueue_style( 'font-awesome_style' );
        wp_enqueue_style( 'ignitepreservation_main_style' );
    }
    add_action('wp_enqueue_scripts', 'ignitepreservation_load_styles');

    // Load Javascript
    function ignitepreservation_load_scripts() {
        ...
        // Custom Scripts
        wp_register_script('ignitepreservation_custom_script', get_template_directory_uri() . '/assets/scripts/script.js', array('jquery'), null, true);
        wp_enqueue_script('ignitepreservation_custom_script');

        if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
            wp_enqueue_script( 'comment-reply' );
        }
    }
    add_action('wp_enqueue_scripts', 'ignitepreservation_load_scripts');
?>
